...
import { getCurrentUser, User } from "../../../store/reducers/userReducer";

const Menu:React.FC = () =>{
    const user: User = useSelector(getCurrentUser as any);
    return(
        <Container>
        <Navbar expand="lg" bg="primary" className=" rounded-3 text-light lead h1 mt-1 px-4 mb-2 d-flex justify-content-between">
            ...
            <Navbar.Brand className="text-dark mx-2"> {user?"logged user:":""} {user?.name}</Navbar.Brand>
            </div>
            <div>
